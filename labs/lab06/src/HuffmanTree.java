...
public class HuffmanTree<Key, E> {
    ...
    private void helper(BinaryNodeKV<Integer, String> node, String bin) {
        if(node.isLeaf())
        {
            codeTable.put(node.getValue(), bin);
        }

        if(!(node.getLeft() == null))
        {
            codeTable.put(node.getLeft().getValue(), bin + "0");
            helper(node.getLeft(), bin + "0");
            codeTable.put(node.getRight().getValue(), bin + "1");
            helper(node.getRight(), bin + "1");
        }
